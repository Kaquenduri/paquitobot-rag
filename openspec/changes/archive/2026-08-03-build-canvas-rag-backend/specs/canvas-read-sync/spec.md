...
(Previously: GET-only access and pagination were required without an explicit pre-transmission method guard on every request.)
...
(Previously: Six-hour and controlled manual synchronization were required without mandatory jitter and distributed per-tenant locking.)
...
(Previously: Replay idempotency and successful watermark advancement were required without explicitly covering retries after partial failure.)
