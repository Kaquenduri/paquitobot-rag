# Delta for Tenant Authentication
...
(Previously: Backend and Canvas credentials were separate without explicitly limiting a personal token to its owning tenant.)
...
(Previously: Plaintext storage and diagnostics were prohibited without explicitly covering exception and API-error output.)
...
(Previously: Server-derived scope did not explicitly cover model-generated SQL and vector metadata as untrusted inputs.)
...
(Previously: Secret-safe errors were required without an explicit all-path redaction boundary that includes logs and unexpected exceptions.)
