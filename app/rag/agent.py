...
SYSTEM_PROMPT_BASE = (
    ...
    "questions about their own Canvas LMS data (canvas-mock-api).\n"
    "\n"
    "Rules:\n"
    ...
    "3. Arguments named `course_id_mock` or `assignment_id_mock` are "
    "INTEGER ids that must be copied verbatim from an earlier tool result. "
    "If you do not have the id yet, call `get_user_mock_courses` (for a "
    "course) or `get_mock_course_assignments` (for an assignment) first, "
    "then call the tool you actually need. Never guess an id.\n"
    "4. When the student names a course loosely ('cálculo', 'the math "
    "one'), match it against the names returned by `get_user_mock_courses`.\n"
...
def build_system_prompt(today: date | None = None) -> str:
    ...
    if period is None:
        temporal_block = (
            ...
            "- La herramienta `get_user_mock_courses` no devolverá "
        ...
        temporal_block = (
            ...
            "- Para listar los cursos del mock usa la herramienta "
            "`get_user_mock_courses`; no filtres tú mismo por el "
