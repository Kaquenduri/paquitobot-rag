...
def default_allow_list() -> AllowList:
    registry = AllowList()
    registry.register("assignments_due_between", "SELECT id, name, due_at FROM assignments WHERE tenant_id = {{tenant_id}} AND due_at BETWEEN :start_at AND :end_at AND deleted_at IS NULL", slots={"tenant_id", "start_at", "end_at"})
    ...
    registry.register("class_score_statistics", "SELECT AVG(score) AS average_score, MIN(score) AS minimum_score, MAX(score) AS maximum_score, COUNT(score) AS scored_count FROM submissions WHERE tenant_id = {{tenant_id}} AND course_id = :course_id AND deleted_at IS NULL", slots={"tenant_id", "course_id"})
    return registry
